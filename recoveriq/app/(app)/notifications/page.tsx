  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-0.5">Real-time alerts across the portfolio</p>
      </div>
      ...
      {loading || !data ? (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h2 className="text-base font-semibold text-gray-900 mb-1">Collector Inactivity</h2>
            <p className="text-xs text-gray-500 mb-4">
              Officers with assigned cases but no logged action in the last 7 days
            </p>
            {data.inactiveCollectors.length === 0 ? (
              <p className="text-sm text-gray-500">No inactive collectors.</p>
            ) : (
              <ul className="space-y-2">
                {data.inactiveCollectors.map((o) => (
                  <li key={o.id} className="flex items-center justify-between text-sm">
                    <span className="text-gray-900">{o.name}</span>
                    <span className="text-gray-500">{o.assignedCases} assigned cases</span>
                  </li>
                ))}
              </ul>
            )}
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h2 className="text-base font-semibold text-gray-900 mb-1">Escalation Events</h2>
            <p className="text-xs text-gray-500 mb-4">Auto-escalated broken promises, last 7 days</p>
            {data.escalationEvents.length === 0 ? (
              <p className="text-sm text-gray-500">No escalation events.</p>
            ) : (
              <ul className="space-y-2">
